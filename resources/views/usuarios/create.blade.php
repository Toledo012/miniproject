<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h1 class="mt-2 text-3xl font-semibold tracking-tight text-slate-950">Nueva cuenta</h1>
        </div>
    </x-slot>
                ...
                <form method="POST" action="{{ route('usuarios.store') }}" class="space-y-6">
                    ...
                    <div class="grid gap-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <x-text-input id="apellidos" name="apellidos" type="text" class="mt-2 block w-full rounded-2xl border-white/80 bg-white/80" :value="old('apellidos')" required />
                    ...
                    <div class="grid gap-6 sm:grid-cols-2">
                        <div>
                            ...
                            <x-text-input id="correo" name="correo" type="email" class="mt-2 block w-full rounded-2xl border-white/80 bg-white/80" :value="old('correo')" required />
                        ...
                        <div>
                            ...
                            <select id="rol" name="rol" class="mt-2 block w-full rounded-2xl border-white/80 bg-white/80">
                                @foreach ($roles as $valor => $etiqueta)
                                    <option value="{{ $valor }}" @selected(old('rol') === $valor)>{{ $etiqueta }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid gap-6 sm:grid-cols-2">
                        <div>
                            ...
                            <x-text-input id="clave" name="clave" type="password" class="mt-2 block w-full rounded-2xl border-white/80 bg-white/80" required />
                            <x-input-error class="mt-2" :messages="$errors->get('clave')" />
                        </div>
                        <div>
                            <x-input-label for="clave_confirmation" value="Confirmar clave" />
                            <x-text-input id="clave_confirmation" name="clave_confirmation" type="password" class="mt-2 block w-full rounded-2xl border-white/80 bg-white/80" required />
                        </div>
                    </div>
